...
export function FollowerRush({
  ...
  // Lead (bold) name cycles through ALL followers in the pool as the count increases,
  // then freezes on the last follower when the countdown completes.
  const nameProgress = Math.max(0, Math.min(1, (fc - INLINE_END) / (EXPLODE_END - INLINE_END)));
  const nameIdx =
    fc < INLINE_END
      ? 0
      : Math.min(pool.length - 1, Math.floor(nameProgress * (pool.length - 1)));
  const leadName = pool[nameIdx].name;
